					...
					<div class="col-md-12">
						...
						<div class="btn btn-success" style="padding: 0px 0px;" >
							<button type="button" class="navbar-toggle"
								...
								<a class="navbar-brand" href="/faces/Admin/muasukien_insert.jsp" style="color: white;text-transform:uppercase;font-size:14px;"> Thêm đợt khuyến mãi</a>
						</div>
					</div>
				</div>
				<hr />
									...
									<table class="table table-striped table-bordered table-hover"
										...
										<%
											...
											<tr class="odd gradeX">
												...
												<a href="/faces/PromotionServlet_View?action=Views&ID_Promotion=<%=pr.getID_Promotion()%>"
													class="btn btn-warning btn-xs">Xem</a></td>
													<td style="text-align:center"><%=pr.getID_Account()%></td>
												
										
												<%-- <td class="center" style="text-align:center"><a href="/faces/Admin/muasukien_edit.jsp?action=Update&ID_Season=<%=cate.getID_Season()%>&Name_Season=<%=cate.getName_Season()%>&DateStart=<%=cate.getDateStart()%>&DateEnd=<%=cate.getDateEnd()%>"
												...
												%>  href="/faces/SeasonServlet_Delete?action=Delete&ID_Season=<%=cate.getID_Season()%>" onclick="return confirm('Bạn có chắc chắn xóa sự kiện <%=cate.getName_Season() %> không ?')"
